...
public class AuthManager implements AuthService {
	...
	public AuthManager(UserService userService, EmployerService employerService, JobseekerService jobseekerService,
			...
			VerificationUtilService verificationUtilService, ResumeService resumeService, UserDao userDao) {
	...
	public DataResult<UserLoginReturnDto> login(LoginDto loginDto) {
		Result result = BusinessRules.run(chekckEmailIsRegistered(loginDto.getEmail()),
				checkUserIsRegistered(loginDto.getEmail()), checkPassword(loginDto));
		if (result.isSuccess()) {
	...
	private Result chekckEmailIsRegistered(String email) {
		if (!this.userService.getUserByEmail(email).isSuccess()) {
